...
class Resnet50_backbone(Model):
    ...
    def forward(self,x):
        ...
        for block_1_idx in range(1,4):
            x=eval(f"self.block_1_{block_1_idx}.forward(x)")
        for block_2_idx in range(1,5):
            x=eval(f"self.block_2_{block_2_idx}.forward(x)")
        for block_3_idx in range(1,7):
            x=eval(f"self.block_3_{block_3_idx}.forward(x)")
        for block_4_idx in range(1,4):
            x=eval(f"self.block_4_{block_4_idx}.forward(x)")
        if(self.pretraining):
